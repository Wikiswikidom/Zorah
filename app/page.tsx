...
export default function Home() {
  return <main id="top" className="site-shell">
    ...
    <section id="collections" className="section"><div className="section-head"><h2 className="section-title">Designed for the day.</h2><p className="section-note">From compact silhouettes to carry-everything shapes, discover pieces built around how you actually move.</p></div><div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>{["Everyday", "Statement", "Custom"].map((name, index) => <a key={name} href={name === "Custom" ? "/custom-orders" : "/collections"} style={{ minHeight: 320, padding: 24, display: "flex", alignItems: "flex-end", background: ["#e5ddd1", "#5a3524", "#173d32"][index], color: index === 0 ? "#111" : "#f7f3ec" }}><span style={{ fontFamily: "var(--font-display), Georgia, serif", fontSize: 42 }}>{name} →</span></a>)}</div></section>
