...
const accountLinks = [
  ...
  { title: 'Waitlist', text: 'Keep an eye on pieces coming back', icon: '◌', href: '/waitlist' },
...
export default async function AccountPage() {
  ...
  const { data: profile } = await admin.from('profiles').select('full_name, role, is_active, phone').eq('id', user.id).maybeSingle()
  if (profile?.role && profile.role !== 'customer') redirect('/admin-login?error=staff_use_admin')
  if (!profile || !profile.is_active) redirect('/login?error=account_unavailable&next=/account')
  const name = profile.full_name || user.user_metadata?.full_name || user.email?.split('@')[0] || 'Customer'
  const firstName = name.split(' ')[0]
  const { data: recentOrders } = await supabase.from('orders').select('id,order_number,total,currency,status,payment_status,created_at').eq('user_id', user.id).order('created_at', { ascending: false }).limit(3)
