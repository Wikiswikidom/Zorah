...
type Customer={user_id:string;email:string|null;full_name:string|null;joined_at:string;successful_orders:number;received_orders:number;products_received:number;total_spent:number;last_order_at:string|null}
...
export default async function AdminCustomersPage({searchParams}:{searchParams?:Promise<{q?:string;sort?:string;status?:string}>}) {
  ...
  const customers=((rawCustomers??[]) as Customer[]).filter(customer=>{
  ...
  const activeCount=registered.filter(customer=>true).length
