...
function parse(body: unknown) {
  ...
  const campaign_type = text(b.campaign_type, 30), status = text(b.status, 20) || 'draft', placement = text(b.placement, 20) || 'landing', discount_type = text(b.discount_type, 20) || 'none'
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { error } = await supabase.from('campaigns').update({ ...parsed.data, updated_by: userId, published_at: parsed.data.status === 'live' ? new Date().toISOString() : null }).eq('id', id)
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    await requireRole(['marketing_admin']); const { id } = await params
    if (!/^[0-9a-f-]{36}$/i.test(id)) return errorResponse('Invalid campaign ID.')
    const supabase = await createClient(); const { error } = await supabase.from('campaigns').delete().eq('id', id)
