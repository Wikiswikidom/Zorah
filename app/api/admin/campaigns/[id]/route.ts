...
function parse(body:unknown){
  ...
  const name=text(b.name,120),slug=text(b.slug,120),title=text(b.title,240),campaign_type=text(b.campaign_type,30),status=text(b.status,20)||'draft',placement=text(b.placement,20)||'landing',discount_type=text(b.discount_type,20)||'none',cta_href=href(b.cta_href)
...
export async function PUT(request:Request,{params}:{params:Promise<{id:string}>}){
  try{
    const{user}=await requireRole(['marketing_admin','ads_admin','content_admin'])
    ...
    if(!parsedData)return errorResponse('Invalid campaign payload.',500)
    const supabase=await createClient()
    const{error}=await supabase.from('campaigns').update({...parsedData,updated_by:user.id,published_at:parsedData.status==='live'?new Date().toISOString():null}).eq('id',id)
    if(error)return errorResponse(error.code==='23505'?'A campaign with this slug already exists.':'Could not update campaign.',error.code==='23505'?409:400)
    return NextResponse.json({ok:true})
  }catch{return errorResponse('Unable to update campaign.',500)}
}
export async function DELETE(_request:Request,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    return NextResponse.json({ok:true})
  }catch{return errorResponse('Unable to delete campaign.',500)}
}
