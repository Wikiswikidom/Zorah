...
export async function GET(){
  try{
    await requireRole(['marketing_admin','content_admin','ads_admin'])
    ...
    const campaigns=await Promise.all((data??[]).map(async c=>{
      const media_url=c.media_path?(await supabase.storage.from('product-media').createSignedUrl(c.media_path,900)).data?.signedUrl??null:null
      return{...c,media_url}
    }))
...
export async function POST(request:Request){
  try{
    ...
    const supabase=await createClient()
    const{data,error}=await supabase.from('campaigns').insert({...parsedData,created_by:user.id,updated_by:user.id,published_at:parsedData.status==='live'?new Date().toISOString():null}).select('id').single()
