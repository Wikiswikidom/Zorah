...
function parse(body: unknown) {
  ...
  const b = body as Record<string, unknown>; const section_key = text(b.section_key, 80), section_type = text(b.section_type, 30), theme = text(b.theme, 20) || 'light', status = text(b.status, 20) || 'draft'
...
export async function POST(request:Request) { try { const {user}=await requireRole(['content_admin','marketing_admin']); const userId=user.id; if(!request.headers.get('content-type')?.toLowerCase().includes('application/json'))return jsonError('JSON request required.',415); let body:unknown; try{body=await request.json()}catch{return jsonError('Invalid JSON request.')}; const parsed=parse(body); if(!('data' in parsed))return jsonError(parsed.error ?? 'Invalid request.'); const parsedData=parsed.data; if(!parsedData)return jsonError('Invalid landing-section payload.',500); const supabase=await createClient(); const {data,error}=await supabase.from('landing_sections').insert({...parsedData,created_by:userId,updated_by:userId,published_at:parsedData.status==='published'?new Date().toISOString():null}).select('id').single(); if(error)return jsonError(error.code==='23505'?'A section with this key already exists.':'Could not create section.',error.code==='23505'?409:400); revalidatePath('/'); return NextResponse.json({id:data.id},{status:201}) } catch { return jsonError('Unable to create landing section.',500) } }
