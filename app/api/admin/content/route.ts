...
function parse(body: unknown) {
  ...
  const section_key = text(b.section_key, 80)
  const section_type = text(b.section_type, 30)
  const theme = text(b.theme, 20) || 'light'
  const status = text(b.status, 20) || 'draft'
  const primary_cta_href = href(b.primary_cta_href)
  const secondary_cta_href = href(b.secondary_cta_href)
  ...
  if (b.scheduled_publish_at) {
    if (typeof b.scheduled_publish_at !== 'string' || Number.isNaN(Date.parse(b.scheduled_publish_at))) return { error: 'Invalid scheduled publish date.' }
    scheduled_publish_at = new Date(b.scheduled_publish_at).toISOString()
    if (status === 'published' && new Date(scheduled_publish_at) > new Date()) return { error: 'Published sections cannot have a future publish time.' }
  }
...
export async function GET() {
  ...
  } catch { return jsonError('Unable to load landing-page content.', 500) }
}
