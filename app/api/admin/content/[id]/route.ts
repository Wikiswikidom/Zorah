...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    await requireRole(['content_admin', 'marketing_admin']); const { id } = await params
    if (!/^[0-9a-f-]{36}$/i.test(id)) return errorResponse('Invalid section ID.')
    const supabase = await createClient(); const { error } = await supabase.from('landing_sections').delete().eq('id', id)
    if (error) return errorResponse('Could not delete section.', 400)
    return NextResponse.json({ ok: true })
  } catch { return errorResponse('Unable to delete landing section.', 500) }
}
