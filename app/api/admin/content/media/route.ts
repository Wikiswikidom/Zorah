...
export async function POST(request:Request){
  try{
    ...
    const upload=await s.storage.from('product-media').upload(path,file,{contentType:file.type,upsert:false,cacheControl:'31536000'})
    if(upload.error)return NextResponse.json({error:'Could not upload landing image.'},{status:400})
    const{data:signed}=await s.storage.from('product-media').createSignedUrl(path,3600)
