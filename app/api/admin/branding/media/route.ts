...
async function validSignature(file: File) {
  ...
  if (file.type === 'image/x-icon' || file.type === 'image/vnd.microsoft.icon') return bytes[0]===0 && bytes[1]===0 && bytes[2]===1 && bytes[3]===0
...
export async function POST(request: Request) {
  try {
    ...
    const extension = file.type==='image/jpeg' ? 'jpg' : file.type==='image/svg+xml' ? 'svg' : file.type.includes('icon') ? 'ico' : file.type.split('/')[1]
    const path = `branding/${kind}-${crypto.randomUUID()}.${extension}`
    const supabase = createAdminClient()
    const upload = await supabase.storage.from(BUCKET).upload(path,file,{contentType:file.type,upsert:false,cacheControl:'31536000'})
