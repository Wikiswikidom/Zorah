...
export async function POST(request:Request){
  try{
    ...
    const type=detected&&TYPES.has(detected)?detected:(TYPES.has(declared)?declared:null)
    if(!type)return errorResponse('Unsupported image. Use PNG, JPG, JPEG, WebP, SVG, AVIF or ICO.')
