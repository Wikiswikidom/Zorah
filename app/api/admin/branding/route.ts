...
export async function GET(){
 try{
  ...
  const rows=data??[]
  const settings=Object.fromEntries(KEYS.map(key=>{const row=rows.find(item=>item.key===key);return[key,{media_path:row?.media_path??null,media_url:publicUrl(supabase,row?.media_path??null),updated_at:row?.updated_at??null}]}))
...
export async function DELETE(request:Request){
 try{
  ...
  if(existing?.media_path)await supabase.storage.from(BUCKET).remove([existing.media_path])
  return NextResponse.json({ok:true})
 }catch(error){console.error('Brand settings DELETE failed',error);return jsonError(error instanceof Error?error.message:'Unable to remove brand artwork.',500)}
}
