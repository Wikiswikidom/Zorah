...
export function CommerceProvider({children}:{children:React.ReactNode}){
  ...
  const loadCustomerData=useCallback(async(id:string,guestCart:CartItem[]=[] )=>{
    try{
      ...
      const remote=(serverCart.data??[]).map((row:any)=>{const snapshot=row?.product_snapshot;if(!isProductSnapshot(snapshot)||!isSafeSlug(row?.product_slug)||typeof row?.variant!=="string")return null;return {product:{...snapshot,slug:row.product_slug},quantity:Math.min(MAX_QUANTITY,Math.max(1,Number(row.quantity)||1)),variant:row.variant.slice(0,100)} as CartItem}).filter((item):item is CartItem=>item!==null);
      if(guestCart.length){
        const merged=[...remote];
  ...
  const syncCartItem=useCallback(async(id:string,item:CartItem)=>{try{await createClient().from("customer_cart").upsert({user_id:id,product_slug:item.product.slug,variant:item.variant,quantity:item.quantity,product_snapshot:item.product},{onConflict:"user_id,product_slug,variant"})}catch(error){console.error("Cart sync failed",error)}},[]);
  const deleteCartItem=useCallback(async(id:string,slug:string,variant:string)=>{try{await createClient().from("customer_cart").delete().eq("user_id",id).eq("product_slug",slug).eq("variant",variant)}catch(error){console.error("Cart delete sync failed",error)}},[]);

  useEffect(()=>{
    let mounted=true;const supabase=createClient();
    (async()=>{const {data}=await supabase.auth.getUser();if(!mounted)return;if(data.user){setUserId(data.user.id);await loadCustomerData(data.user.id)}else{setCart(readStorage(STORAGE_KEYS.cart,[],isCart));setWishlist(readStorage(STORAGE_KEYS.wishlist,[],isWishlist));setRecentlyViewed(readStorage(STORAGE_KEYS.recentlyViewed,[],isRecentlyViewed))}if(mounted)setHydrated(true)})();
    const{data:subscription}=supabase.auth.onAuthStateChange((_event,session)=>{if(!mounted)return;const nextId=session?.user?.id??null;if(nextId){const guestCart=readStorage(STORAGE_KEYS.cart,[],isCart);setUserId(nextId);loadCustomerData(nextId,guestCart)}else{setUserId(null);setCart([]);setWishlist([]);setRecentlyViewed([]);try{window.localStorage.removeItem(STORAGE_KEYS.cart);window.localStorage.removeItem(STORAGE_KEYS.wishlist);window.localStorage.removeItem(STORAGE_KEYS.recentlyViewed)}catch{}}});
  ...
  const addToBag=useCallback((product:Product,quantity=1,variant="Default")=>{if(!isSafeSlug(product.slug))return;const q=Math.min(MAX_QUANTITY,Math.max(1,Math.floor(Number(quantity)||1))),v=String(variant).slice(0,100);setCart(current=>{const index=current.findIndex(item=>item.product.slug===product.slug&&item.variant===v);const next=index<0?{product,quantity:q,variant:v}:{...current[index],quantity:Math.min(MAX_QUANTITY,current[index].quantity+q)};if(index<0&&current.length>=MAX_CART_ITEMS)return current;if(userId)void syncCartItem(userId,next);return index<0?[...current,next]:current.map((item,i)=>i===index?next:item)})},[syncCartItem,userId]);
  const removeFromBag=useCallback((slug:string,variant="Default")=>{if(!isSafeSlug(slug))return;setCart(current=>current.filter(item=>!(item.product.slug===slug&&item.variant===variant)));if(userId)void deleteCartItem(userId,slug,variant)},[deleteCartItem,userId]);
  const setQuantity=useCallback((slug:string,quantity:number,variant="Default")=>{if(!isSafeSlug(slug))return;const q=Math.floor(Number(quantity));if(!Number.isFinite(q)||q<=0){setCart(current=>current.filter(item=>!(item.product.slug===slug&&item.variant===variant)));if(userId)void deleteCartItem(userId,slug,variant);return}setCart(current=>current.map(item=>{if(item.product.slug!==slug||item.variant!==variant)return item;const next={...item,quantity:Math.min(MAX_QUANTITY,q)};if(userId)void syncCartItem(userId,next);return next}))},[deleteCartItem,syncCartItem,userId]);
  const clearBag=useCallback(()=>{setCart([]);if(userId){void (async()=>{try{await createClient().from("customer_cart").delete().eq("user_id",userId)}catch(error){console.error("Cart clear sync failed",error)}})()}},[userId]);

  const toggleWishlist=useCallback((slug:string)=>{if(!isSafeSlug(slug))return;setWishlist(current=>current.includes(slug)?current.filter(item=>item!==slug):[...current,slug].slice(-200));if(userId){(async()=>{const supabase=createClient();const productId=await productIdForSlug(slug);if(!productId)return;try{const {data}=await supabase.from("customer_wishlists").select("product_id").eq("user_id",userId).eq("product_id",productId).maybeSingle();if(data)await supabase.from("customer_wishlists").delete().eq("user_id",userId).eq("product_id",productId);else await supabase.from("customer_wishlists").insert({user_id:userId,product_id:productId});}catch(error){console.error("Wishlist sync failed",error)}})()}},[userId]);
  const isWishlisted=useCallback((slug:string)=>wishlist.includes(slug),[wishlist]);
  const trackViewed=useCallback((slug:string)=>{if(!isSafeSlug(slug))return;setRecentlyViewed(current=>[slug,...current.filter(item=>item!==slug)].slice(0,MAX_RECENT));if(userId){(async()=>{const supabase=createClient();const productId=await productIdForSlug(slug);if(!productId)return;try{await supabase.from("customer_recently_viewed").upsert({user_id:userId,product_id:productId,viewed_at:new Date().toISOString()},{onConflict:"user_id,product_id"});}catch(error){console.error("Recently viewed sync failed",error)}})()}},[userId]);
