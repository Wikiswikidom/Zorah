...
export function ProductCard({slug,name,price,priceValue=0,tone="ivory",imageUrl,badge,category,availability,colors=[],variantDetails=[]}:ProductCardProps){
 ...
 return <article className="product-card">
  <div className="product-media-wrap"><a href={slug?`/products/${slug}`:"/shop"} aria-label={`View ${name}`}><div className="product-media">{imageUrl?<img src={imageUrl} alt={`${name} leather handbag by Zorah`} loading="lazy" decoding="async" className="h-full w-full object-cover"/>:<div className="product-placeholder" aria-hidden="true">Z</div>}{badge&&<span className="product-badge">{badge}</span>}</div></a>{slug&&<button type="button" className={`wishlist-button ${saved?"is-saved":""}`} onClick={()=>toggleWishlist(slug)} aria-label={saved?`Remove ${name} from wishlist`:`Save ${name} to wishlist`} aria-pressed={saved}>{saved?"♥":"♡"}</button>}</div>
