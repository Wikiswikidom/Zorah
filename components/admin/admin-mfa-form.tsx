...
export function AdminMfaForm({ nextPath, email }: { nextPath: string; email: string }) {
  ...
  useEffect(() => {
    ...
    async function prepare() {
      ...
      const pending = result.data.totp.find((factor) => factor.status !== 'verified')
      if (pending) {
      ...
      <section className="mx-auto max-w-md rounded-[2rem] border border-white/10 bg-white/[.05] p-7 shadow-2xl sm:p-9">
        ...
        {mode === 'enroll' && (
          <div className="mt-7 space-y-5">
            <div className="rounded-2xl bg-white p-4">
              {qr
                ? <img src={`data:image/svg+xml;utf8,${encodeURIComponent(qr)}`} alt="Authenticator setup QR code" className="mx-auto h-56 w-56" />
                : <div className="h-56" />}
            </div>
            <p className="text-xs leading-5 text-white/55">Scan the QR code with an authenticator app such as Google Authenticator or Microsoft Authenticator.</p>
        ...
        {(mode === 'enroll' || mode === 'verify') && (
          <div className="mt-7">
            ...
            <button type="button" disabled={busy || code.length !== 6} onClick={verify} className="mt-4 w-full rounded-xl bg-white px-5 py-3 text-sm font-semibold text-black disabled:cursor-not-allowed disabled:opacity-40">
              {busy ? 'Verifying…' : mode === 'enroll' ? 'Enable secure access' : 'Continue to Commerce Studio'}
            </button>
          </div>
        )}

        <button type="button" onClick={() => supabase.auth.signOut().then(() => router.replace('/admin-login'))} className="mt-5 w-full text-xs text-white/45 hover:text-white">
          Cancel and sign out
        </button>
      </section>
    </main>
