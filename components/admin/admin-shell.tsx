    ...
    <div className="zorah-admin-shell">
      <aside className="zorah-admin-sidebar">
        ...
        <nav className="zorah-admin-nav" aria-label="Administration">
          {groups.map((group) => <div className="zorah-admin-nav-group" key={group.label}><p>{group.label}</p>{group.items.map(([label, href, icon]) => {
            const active = href === "/admin" ? pathname === href : pathname === href || pathname.startsWith(`${href}/`);
            return <Link key={href} href={href} className={`zorah-admin-nav-link ${active ? "is-active" : ""}`}><span className="zorah-admin-nav-icon" aria-hidden>{icon}</span><span>{label}</span></Link>;
          })}</div>)}
        </nav>
        <div className="zorah-admin-sidebar-foot"><Link href="/" className="zorah-admin-quiet-link">View brand site ↗</Link><Link href="/shop" className="zorah-admin-quiet-link">View storefront ↗</Link></div>
      </aside>
      <div className="zorah-admin-main"><div className="zorah-admin-mobilebar"><Link href="/admin" className="zorah-admin-wordmark">ZORAH</Link><Link href="/shop" className="zorah-admin-mobile-store">Storefront ↗</Link></div>{children}</div>
    </div>
  );
}
