...
export function EditorialMotion() {
  ...
  useEffect(() => { const timer = window.setInterval(() => setActive((value) => (value + 1) % heroSlides.length), 5000); return () => window.clearInterval(timer); }, []);
  useLayoutEffect(() => {
  ...
  return (
    <div className="zorah-site">
      <header className="zorah-header"><div className="zorah-header-inner">
        ...
        <nav className="zorah-nav zorah-nav-right"><a href="/login">Account</a><a className="zorah-shop-link" href="/login">Sign in / Create account <span>↗</span></a></nav>
      </div></header>
      ...
      <main>
        <section className="zorah-hero" aria-labelledby="zorah-title">
          ...
          <div className="zorah-hero-inner">
            <div className="zorah-hero-copy"><p className="zorah-eyebrow zorah-reveal">ZORAH HANDBAGS · LAGOS</p><h1 id="zorah-title" className="zorah-display zorah-reveal">Carry your<br /><em>point of view.</em></h1><p className="zorah-lede zorah-reveal">Leather handbags made in Lagos, designed for the life you actually live.</p><div className="zorah-actions zorah-reveal"><a className="zorah-btn zorah-btn-dark" href="/login">Enter Zorah <span>→</span></a><a className="zorah-link" href="#collections">Explore Zorah</a></div></div>
        ...
        <section id="collections" className="zorah-collection-strip"><div className="zorah-section-meta zorah-scroll-reveal"><span>01 — The edit</span><span>Discover the house</span></div><div className="collection-head"><h2 className="zorah-display zorah-scroll-reveal">The pieces<br /><em>people notice.</em></h2><a className="zorah-underlink zorah-scroll-reveal" href="/login">Enter your account <span>→</span></a></div><div className="collection-rail" role="list">{heroSlides.map((slide, index) => <a className="collection-card zorah-scroll-reveal" href="/login" role="listitem" key={slide.number}><div className={`collection-image collection-image-${slide.tone}`}><span>0{index + 1}</span><BagIllustration tone={slide.tone} small /></div><div className="collection-card-copy"><strong>{slide.title}</strong><span>{slide.label}</span></div></a>)}</div></section>

        <section className="zorah-wear-section"><div className="zorah-wear-intro"><p className="zorah-eyebrow zorah-scroll-reveal">02 — As seen in Lagos</p><h2 className="zorah-display zorah-scroll-reveal">Worn<br /><em>your way.</em></h2><a className="zorah-underlink zorah-scroll-reveal" href="/login">Continue to Zorah <span>↗</span></a></div><div className="wear-rail" role="list">{wearSlides.map((item, index) => <article className="wear-card zorah-scroll-reveal" role="listitem" key={item.name}><div className={`wear-image wear-image-${item.tone}`}><span>IMAGE SLOT</span><BagIllustration tone={item.tone} small /><b>0{index + 1}</b></div><div className="wear-copy"><strong>{item.name}</strong><span>{item.meta}</span></div></article>)}</div></section>

        <section id="craft" className="zorah-story-grid"><div className="story-image zorah-image-drift"><div className="story-image-placeholder"><span>IMAGE SLOT · CRAFT</span><BagIllustration tone="green" /><small>Replace with atelier / maker photography</small></div></div><div className="story-copy"><p className="zorah-eyebrow zorah-scroll-reveal">03 — The making</p><h2 className="zorah-display zorah-scroll-reveal">Made in Lagos.<br /><em>Made to last.</em></h2><p className="zorah-body zorah-scroll-reveal">Cut, assembled and finished with an eye for the details you feel before you notice them.</p><div className="craft-mini-grid zorah-scroll-reveal"><span><b>01</b>Leather</span><span><b>02</b>Form</span><span><b>03</b>Finish</span></div><a className="zorah-underlink zorah-scroll-reveal" href="/login">Continue to Zorah <span>→</span></a></div></section>
